...
def main(args):
    ...
    for r in ratio:
        ...
        # pretrin Sc
        if args.pretrain:
            df = find_single_best_hyperparameters(table_path + args.problem + 'ae_tuning_r.csv', args.main_metric)
            ...
            if yBias is not None:
                np.save('latent/pretrain/{2}/yB_{0}_{1}_{3}'.format(df['model'], df['rank'], args.problem, args.scene),
                        yBias)
            if X is not None:
                np.save('latent/pretrain/{2}/X_{0}_{1}_{3}'.format(df['model'], df['rank'], args.problem, args.scene),
                        X)
            if xBias is not None:
                np.save('latent/pretrain/{2}/xB_{0}_{1}_{3}'.format(df['model'], df['rank'], args.problem, args.scene),
                        xBias)
