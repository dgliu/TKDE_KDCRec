...
def ae(matrix_train, matrix_valid, embeded_matrix=np.empty(0), matrix_utrain=None,
       iteration=100, lam=0.01, rank=200, batch_size=256, learning_rate=1e-4, optimizer="Adam", seed=0,
       source=None, gpu_on=False, metric='AUC', topK=50, is_topK=False, **unused):
    progress = WorkSplitter()

    progress.section("AE: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("AE: Training")
    ...
    model = AE(n, rank, batch_size, lamb=lam, learning_rate=learning_rate, optimizer=Regularizer[optimizer],
               gpu_on=gpu_on)
