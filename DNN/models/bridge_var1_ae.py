    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('bridge-ae'):
            ...
            with tf.variable_scope('counter_factual_loss'):
                ...
                self.cf_loss = 0.5 * tf.reduce_mean(
                    tf.square((self.prediction - self.unif_prediction) * self.sample_idx)) + 0.5 * tf.reduce_mean(
                    tf.square((self.prediction - self.norm_prediction) * self.sample_idx))
...
def bridgeae1(matrix_train, matrix_valid, embeded_matrix=np.empty(0), matrix_utrain=None,
