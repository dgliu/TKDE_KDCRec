...
def main(args):
    ...
    for r in ratio:
        ...
        # pretrin Sc and St
        if args.pretrain:
            df = find_single_best_hyperparameters(table_path + args.problem + 'unif_mf_tuning_r.csv', args.main_metric)
            ...
            np.save('latent/pretrain/{2}/{3}_U_{0}_{1}_{4}'.format(df['model'], df['rank'], args.problem, 'unif',
                                                                   args.scene), RQ)
            np.save('latent/pretrain/{2}/{3}_V_{0}_{1}_{4}'.format(df['model'], df['rank'], args.problem, 'unif',
                                                                   args.scene), Y)
            if yBias is not None:
                np.save(
                    'latent/pretrain/{2}/{3}_vB_{0}_{1}_{4}'.format(df['model'], df['rank'], args.problem, 'unif',
                                                                    args.scene), yBias)
            if X is not None:
                np.save(
            ...
            if xBias is not None:
                np.save(
                    'latent/pretrain/{2}/{3}_uB_{0}_{1}_{4}'.format(df['model'], df['rank'], args.problem, 'unif',
                                                                    args.scene), xBias)
