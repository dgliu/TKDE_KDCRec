    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('bridge-mf'):
            ...
            with tf.variable_scope("counter_factual_loss"):
                ...
                norm_users = tf.nn.embedding_lookup(self.norm_user_embeddings, self.sampled_user_idx)
                norm_users_bias = tf.nn.embedding_lookup(self.norm_user_bias_embeddings, self.sampled_user_idx)
                norm_items = tf.nn.embedding_lookup(self.norm_item_embeddings, self.sampled_item_idx)
                norm_item_bias = tf.nn.embedding_lookup(self.norm_item_bias_embeddings, self.sampled_item_idx)

                norm_label = tf.reduce_sum(tf.multiply(norm_users, norm_items),
                                           axis=1) + norm_users_bias + norm_item_bias

                mf_loss = 0.5 * tf.reduce_mean(tf.square(unif_label - x_ij)) + 0.5 * tf.reduce_mean(
                    tf.square(norm_label - x_ij))
...
def bridgemf1(matrix_train, matrix_valid, embeded_matrix=np.empty(0), matrix_utrain=None,
    ...
    progress.section("Bridge-MF: Load the variables trained on S_c/S_t")
    if scene == 'r':
