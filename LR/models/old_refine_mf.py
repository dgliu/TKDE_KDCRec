...
def oldrefinemf(matrix_train, matrix_valid, embeded_matrix=np.empty(0), matrix_utrain=None,
                iteration=100, lam=0.01, confidence=0.9, rank=200, batch_size=500, learning_rate=1e-3,
                optimizer="LAdam", seed=0, gpu_on=False, problem=None, scene='r', metric='AUC', topK=50, is_topK=False,
                **unused):
    progress = WorkSplitter()

    progress.section("Old-Refine-MF: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("Old-Refine0MF: Load the variables trained on S_c/S_t")
    if scene == 'r':
    ...
    progress.section("Old-Refine-MF: Training")
