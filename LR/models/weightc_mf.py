        ...
        with tf.variable_scope('weightc-mf'):
            ...
            with tf.variable_scope('mf_loss'):
                ...
                if self._mode == 'global':
                    ...
                    mf_loss = self._confidence * self.mark * mf_loss + (1 - self.mark) * mf_loss
...
def weightcmf(matrix_train, matrix_valid, embeded_matrix=np.empty(0), matrix_utrain=None,
              iteration=100, lam=0.01, confidence=0.9, rank=200, batch_size=500, learning_rate=1e-3, optimizer="LAdam",
              seed=0, gpu_on=False, problem=None, scene='r', metric='AUC', topK=50, is_topK=False, measure='sigmoid',
              mode='global', **unused):
    progress = WorkSplitter()

    progress.section("WeightC-MF: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("WeightC-MF: Load the variables trained on S_c/S_t")
    if scene == 'r':
    ...
    progress.section("WeightC-MF: Training")
    ...
    model = WeightCMF(m, n, rank, batch_size, lamb=lam, learning_rate=learning_rate, measure=measure, mode=mode,
                      confidence=confidence, optimizer=Regularizer[optimizer], gpu_on=gpu_on, init_unif_U=unif_RQ,
                      init_unif_V=unif_Y, init_unif_uBias=unif_uBias, init_unif_vBias=unif_vBias)
