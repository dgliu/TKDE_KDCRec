...
class IPSAE(object):
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('ips-ae'):
            # Placehoder
            self.inputs = tf.placeholder(tf.float32, (None, self._input_dim))
            self.weight = tf.placeholder(tf.float32, (None, self._input_dim))
            ...
            with tf.variable_scope('loss'):
                ...
                mf_loss = self.weight * tf.square(self.inputs - self.prediction * mask)
    ...
    def get_batches(rating_matrix, marks, batch_size):
        ...
        while remaining_size > 0:
            if remaining_size < batch_size:
                ...
                weight.append(marks[batch_index * batch_size:])
            else:
                batches.append(rating_matrix[batch_index*batch_size:(batch_index+1)*batch_size])
                weight.append(marks[batch_index * batch_size:(batch_index + 1) * batch_size])
            batch_index += 1
            remaining_size -= batch_size
        return batches, weight

    def train_model(self, matrix_train, marks, matrix_valid, epoch=100, metric='AUC', topK=50, is_topK=False):
        batches, weight = self.get_batches(matrix_train, marks, self._batch_size)
        ...
        for i in tqdm(range(epoch)):
            for step in range(len(batches)):
                feed_dict = {self.inputs: batches[step].todense(),
                             self.weight: weight[step].todense()}
    ...
    def get_RQ(self, rating_matrix, marks):
        batches, _ = self.get_batches(rating_matrix, marks, self._batch_size)
        RQ = []
        for step in range(len(batches)):
            feed_dict = {self.inputs: batches[step].todense()}
...
def ipsae(matrix_train, matrix_valid, embeded_matrix=np.empty(0), matrix_utrain=None,
          iteration=100, lam=0.01, rank=200, batch_size=500, learning_rate=1e-4, optimizer="Adam", seed=0,
          gpu_on=False, metric='AUC', topK=50, is_topK=False, **unused):
    progress = WorkSplitter()

    progress.section("IPS-AE: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("IPS-AE: Calculating Propensity Score")
    ...
    P_Y = np.array([np.sum(matrix_utrain == -1) / matrix_utrain.count_nonzero(),
                    1 - np.sum(matrix_utrain == -1) / matrix_utrain.count_nonzero()])
    invP = 1 / (P_YO * P_O / P_Y)

    progress.section("IPS-AE: Training")
    matrix_input = matrix_train + matrix_utrain

    marks = csr_matrix(matrix_input.shape)
    marks[(matrix_input == 1).nonzero()] = invP[1]
    marks[(matrix_input == -1).nonzero()] = invP[0]

    if embeded_matrix.shape[0] > 0:
        matrix_input = vstack((matrix_input, embeded_matrix.T))

    m, n = matrix_input.shape
    model = IPSAE(n, rank, batch_size, lamb=lam, learning_rate=learning_rate, optimizer=Regularizer[optimizer],
                  gpu_on=gpu_on)
