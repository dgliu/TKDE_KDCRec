...
class IPSMF(object):
    ...
    def train_model(self, matrix_train, matrix_valid, invP, epoch=100, metric='AUC', topK=50, is_topK=False):
        ...
        for i in tqdm(range(epoch)):
            batches = self.get_batches(user_item_pairs, matrix_train, invP, self._batch_size)
...
def ipsmf(matrix_train, matrix_valid, embeded_matrix=np.empty(0), matrix_utrain=None,
          iteration=100, lam=0.01, rank=200, batch_size=500, learning_rate=1e-3, optimizer="LAdam", seed=0,
          gpu_on=False, metric='AUC', topK=50, is_topK=False, **unused):
    progress = WorkSplitter()

    progress.section("IPS-MF: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("IPS-MF: Calculating Propensity Score")
    ...
    progress.section("IPS-MF: Training")
    matrix_input = matrix_train + matrix_utrain
    if embeded_matrix.shape[0] > 0:
        matrix_input = vstack((matrix_input, embeded_matrix.T))

    m, n = matrix_input.shape
    model = IPSMF(m, n, rank, batch_size, lamb=lam, learning_rate=learning_rate, optimizer=Regularizer[optimizer],
                  gpu_on=gpu_on)

    RQ, X, xBias, Y, yBias = model.train_model(matrix_input, matrix_valid, invP, iteration, metric, topK, is_topK)
