...
class WeightCAE(object):
            ...
            with tf.variable_scope('loss'):
                ...
                if self._mode == 'global':
                    self._loss = tf.reduce_mean(
                        self._confidence * self.mask * mf_loss +
                        (1 - self.mask) * mf_loss) + self._lamb * tf.reduce_mean(l2_loss)
                else:
                    self.unif_encoded = tf.nn.relu(
                        tf.matmul(self.inputs, self.unif_encode_weights) + self.unif_encode_bias)
    ...
    def train_model(self, matrix_train, marks, matrix_valid, epoch=100, metric='AUC', topK=50, is_topK=False):
        ...
        for i in tqdm(range(epoch)):
            batches, marks_batches = self.get_batches(matrix_train, marks, self._batch_size)
...
def weightcae(matrix_train, matrix_valid, embeded_matrix=np.empty(0), matrix_utrain=None,
              iteration=100, lam=0.01, confidence=0.9, rank=200, batch_size=256, learning_rate=1e-4, optimizer="Adam",
              seed=0, gpu_on=False, problem=None, scene='r', metric='AUC', topK=50, is_topK=False, measure='exp',
              mode='global', **unused):
    progress = WorkSplitter()

    progress.section("WeightC-AE: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("WeightC-AE: Load the variables trained on S_c/S_t")
    if scene == 'r':
    ...
    progress.section("WeightC-AE: Training")
    ...
    model = WeightCAE(n, rank, batch_size, lamb=lam, learning_rate=learning_rate, measure=measure, mode=mode,
                      confidence=confidence, optimizer=Regularizer[optimizer], gpu_on=gpu_on, init_unif_X=unif_X,
                      init_unif_Y=unif_Y, init_unif_xBias=unif_xBias, init_unif_yBias=unif_yBias)
